    ...
    for (size_t i = 0; i < d->size; i++)
    {
        ...
        while (curr)
        {
            if (strchr(curr->key, ':') == NULL)
            {
                if (foundsec == n)
                {
                    return curr->key; /* 第 n 個 section 找到了 */
                }
                foundsec++;
            }
            curr = curr->next;
...
void iniparser_dump_ini(const struct dictionary *d, FILE *f)
{
    ...
    size_t nsec = iniparser_getnsec(d);
    char   escaped[(ASCIILINESZ * 2) + 2] = "";

    /*------------------------------------------------------------*/
    /*  沒有任何 section：直接列出所有「key = value」               */
    /*------------------------------------------------------------*/
    if (nsec < 1) {
        for (size_t i = 0; i < d->size; i++) {
            struct bucket *curr = d->table[i];
            while (curr) {
                escape_value(escaped, curr->value);
                fprintf(f, "%s = \"%s\"\n",
                        curr->key,
                        escaped);
                curr = curr->next;
    ...
    /*------------------------------------------------------------*/
    /*  有 section：逐一呼叫 dumpsection_ini()                    */
    /*------------------------------------------------------------*/
    for (size_t i = 0; i < nsec; i++) {
        const char *secname = iniparser_getsecname(d, (int)i);
...
void iniparser_dumpsection_ini(const struct dictionary *d, const char *s, FILE *f)
{
    ...
    if (strlen(s) >= ASCIILINESZ) return;

    /* section 標頭 */
    fprintf(f, "\n[%s]\n", s);

    /* 準備前綴 "section:" 供比對用 */
    char prefix[ASCIILINESZ + 1];
    ...
    for (size_t i = 0; i < d->size; i++) {
        ...
        while (curr) {
            /* 判斷是否屬於該 section */
            if (strncmp(curr->key, prefix, prelen) == 0) {
                escape_value(escaped, curr->value);   /* 跳脫字串中的 \ 與 " */
                fprintf(f, "%-30s = \"%s\"\n",
                        curr->key + prelen,           /* 冒號後面的部分 */
                        escaped);
            }
            curr = curr->next;
        }
...
int iniparser_getsecnkeys(const struct dictionary *d, const char *s)
{
    ...
    /* 若該 section 不存在，直接回傳 0 */
    if (!iniparser_find_entry(d, s))
    {
        return 0;
    }

    /* 準備比對用的前綴字串 "section:"（已轉成小寫） */
    char keym[ASCIILINESZ + 1];
    size_t seclen = strlen(s);
    ...
    for (size_t i = 0; i < d->size; i++)
    {
        ...
        {
            /*
             * 若 key 以 "section:" 為前綴就累計。
             * seclen+1 因為包含 ':'。
             */
            if (!strncmp(curr->key, keym, seclen + 1))
            {
                nkeys++;
            }
            curr = curr->next;
 ...
 */
/*--------------------------------------------------------------------------*/
const char **iniparser_getseckeys(const struct dictionary *d,
                                  const char *s,
                                  const char **keys)
{
    if (d == NULL || s == NULL || keys == NULL)
    {
        return NULL;
    }

    /* 若 section 根本不存在，直接返回 NULL */
    if (!iniparser_find_entry(d, s))
    {
        return NULL;
    }
    ...
    /* 準備小寫前綴 "section:" 以便比對 */
    char keym[ASCIILINESZ + 1];
    strlwc(s, keym, sizeof(keym)); /* to lower case */
    size_t seclen = strlen(keym);
    keym[seclen] = ':'; /* append ':' */
    keym[seclen + 1] = '\0';

    int nk = 0; /* 寫入 keys[] 的索引 */

    /* 兩層：先掃 bucket 陣列，再沿鏈表 */
    for (size_t i = 0; i < d->size; i++)
    {
        struct bucket *curr = d->table[i];
        while (curr)
        {
            /* 若以 "section:" 為前綴，就加入結果陣列 */
            if (strncmp(curr->key, keym, seclen + 1) == 0)
            {
                keys[nk++] = curr->key; /* 直接存指標，不複製字串 */
            }
            curr = curr->next;
        }
    }

    return (nk > 0) ? keys : NULL; /* 無鍵時回傳 NULL 與舊版一致 */
}
...
long int iniparser_getlongint(const struct dictionary *d, const char *key, long int notfound)
{
    ...
    if (str == NULL || str == INI_INVALID_KEY)
        return notfound;
